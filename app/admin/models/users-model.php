...
class AdvisoriesModel {
    ...
    public function assignAdvisoryTeacher($data) {
        try {
            ...
            // Insert or update teacher role to advisory
            $roleQuery = "INSERT INTO {$this->teacher_roles_table} (teacher_id, role_type) 
                         VALUES (:teacher_id, 'advisory')
                         ON DUPLICATE KEY UPDATE role_type = 'advisory'";
            $roleStmt = $this->conn->prepare($roleQuery);
            $roleStmt->execute([':teacher_id' => $data['teacher_id']]);
            
            // Insert advisory class
            $query = "INSERT INTO {$this->advisory_table} 
                     (teacher_id, advisory_name, created_at) 
                     VALUES (:teacher_id, :advisory_name, NOW())";
            ...
            $this->conn->commit();
            return ['success' => true, 'message' => 'Advisory teacher assigned successfully!'];
    ...
    public function getAdvisoryTeachers() {
        try {
            $query = "SELECT 
                     ...
                     GROUP BY a.advisory_id, a.teacher_id, u.name, u.email, a.advisory_name, tr.role_type, a.created_at
    ...
    /**
     * Get all students (for assignment modal) - includes grade level
     */
    public function getAllStudents() {
        try {
            // First try to get students with their current grade from assignments
            $query = "SELECT DISTINCT
                        u.user_id, 
                        u.name, 
                        u.lrn,
                        COALESCE(aa.grade_level, '7') as grade_level
