...
function openStudentProfileModal(studentId) {
  ...
  fetch(window.location.href, {
    ...
    .catch(() => {
      loadingState.classList.add("hidden");
      showToast("An error occurred while loading student profile.", "error");
      modal.classList.add("hidden");
      document.body.style.overflow = "auto";
    });
}
...
function populateStudentProfile(student) {
  // Parse name into parts: First [MI] Last
  const nameParts = (student.name || "").trim().split(/\s+/);
  let firstName = "";
  let lastName = "";
  let mi = "";

  // Find middle initial (1-2 chars or has period)
  ...
  document.getElementById("profileGradeBadge").textContent = student.current_grade
    ? `Grade ${student.current_grade}`
    : "Unassigned";
  document.getElementById("profileLrnDisplay").textContent = student.lrn || "—";
  document.getElementById("profileAdvisoryDisplay").textContent =
    student.advisory_name || "Not Assigned";
  document.getElementById("profileAdviserDisplay").textContent =
    student.teacher_name || "—";

  // Profile image
  const avatarIcon = document.getElementById("profileAvatarIcon");
  const avatarImg = document.getElementById("profileAvatarImg");
  if (student.profile_pix) {
  ...
  // Right panel — all editable inputs
  ...
  // Store original profile_pix for later
  ...
  document.getElementById("profileYearLevel").textContent = gradeText;
  document.getElementById("profileSection").textContent = advisoryText;
  document.getElementById("profileAdviser").textContent = adviserText;
}
...
function saveStudentProfile() {
  ...
  if (fileInput.files.length > 0) {
    ...
    fetch(window.location.href, {
      ...
      .catch(() => showToast("An error occurred while uploading image.", "error"));
  } else {
    saveProfileData(studentId, firstName, lastName, mi, lrn, contact, address, profilePicPath);
  }
}
...
function saveProfileData(studentId, firstName, lastName, mi, lrn, contact, address, profilePicPath) {
  ...
  fetch(window.location.href, { method: "POST", body: formData })
    ...
    .catch(() => showToast("An error occurred while saving.", "error"));
}
...
function displayFilteredStudents() {
  ...
  filteredStudents.forEach((student) => {
    ...
    // Build grade options 7-12, selected = current grade
    let gradeOptions = "";
    for (let g = 7; g <= 12; g++) {
    ...
    html += `
      <tr class="student-row hover:bg-gray-50 transition" data-grade="${student.grade_level}" data-student-id="${student.user_id}">
        ...
        <td class="py-3 px-6">
          ...
          <div class="text-xs text-gray-500 mt-0.5"><i class="fa-solid fa-layer-group mr-1 text-xs"></i>Grade ${student.grade_level}</div>
        </td>
        <td class="py-3 px-6 text-sm text-gray-700 font-mono">${escapeHtml(student.lrn || "—")}</td>
        <td class="py-3 px-6">
          <span class="px-3 py-1 ${gradeClass} rounded-lg text-xs font-bold"><i class="fa-solid fa-graduation-cap mr-1"></i>Grade ${student.grade_level}</span>
        </td>
        <td class="py-3 px-6">
...
// Called when grade dropdown changes in Assign Students modal
function updateStudentGradeInModal(selectEl, studentId) {
  const newGrade = selectEl.value;

  // Update the hidden grade_levels input so the assign form uses the new grade
  const hiddenInput = document.getElementById(`gradeHidden_${studentId}`);
  if (hiddenInput) hiddenInput.value = newGrade;

  // Update the row's data-grade attribute so filters still work
  const row = selectEl.closest("tr");
  if (row) row.setAttribute("data-grade", newGrade);

  // Update the current grade badge display in the same row
  ...
  if (badge) {
    ...
    badge.innerHTML = `<i class="fa-solid fa-graduation-cap mr-1"></i>Grade ${newGrade}`;
  }

  // Also update the subtitle under student name
  const subtitle = row ? row.querySelector("td:nth-child(2) .text-xs") : null;
  if (subtitle) subtitle.innerHTML = `<i class="fa-solid fa-layer-group mr-1 text-xs"></i>Grade ${newGrade}`;

  // Persist to server so the grade is remembered
  const formData = new FormData();
  ...
  fetch(window.location.href, { method: "POST", body: formData })
    ...
    .then((data) => {
      if (data.success) {
        // Update in-memory student list so filter refresh keeps new grade
...
function renderPagination(totalItems) {
  ...
  if (startPage > 1) {
    html += `<button onclick="changePage(1)" class="w-10 h-10 flex items-center justify-center rounded-xl bg-white border border-gray-200 text-gray-600 hover:bg-gray-100 font-semibold transition"><i class="fa-solid fa-step-backward"></i></button>`;
  ...
  if (endPage < totalPages) {
    ...
    html += `<button onclick="changePage(${totalPages})" class="w-10 h-10 flex items-center justify-center rounded-xl bg-white border border-gray-200 text-gray-600 hover:bg-gray-100 font-semibold transition"><i class="fa-solid fa-step-forward"></i></button>`;
  }
    ...
    <tr>
      <td colspan="4" class="py-12 text-center">
        <div class="flex flex-col items-center justify-center text-gray-400">
          ...
          <p class="text-sm font-medium text-gray-600"><i class="fa-solid fa-hourglass-end mr-2"></i>Loading Available Students...</p>
        </div>
      </td>
    </tr>
  `;
  ...
  fetch(window.location.href, { method: "POST", body: formData })
    ...
    .then((data) => {
      if (data.success) {
        ...
        tbody.innerHTML = `<tr><td colspan="4" class="py-12 text-center text-gray-400"><i class="fa-solid fa-exclamation-triangle text-4xl mb-3 opacity-20"></i><p class="text-sm font-medium text-gray-600">Error Loading Students</p></td></tr>`;
      }
    })
    .catch(() => {
      tbody.innerHTML = `<tr><td colspan="4" class="py-12 text-center text-gray-400"><i class="fa-solid fa-exclamation-triangle text-4xl mb-3 opacity-20"></i><p class="text-sm font-medium text-gray-600">Error Loading Students</p></td></tr>`;
    ...
    html += `
      <tr class="hover:bg-gray-50 transition">
        <td class="py-4 px-6" colspan="3">
          <div class="text-sm font-medium text-gray-900"><i class="fa-solid fa-user-tie mr-2 text-[#043915]"></i>${escapeHtml(teacher.teacher_name)}</div>
          <div class="text-xs text-gray-500"><i class="fa-solid fa-envelope mr-1 text-gray-400"></i>${escapeHtml(teacher.teacher_email)}</div>
        </td>
        <td class="py-4 px-6" colspan="2">
          <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-purple-100 text-purple-800"><i class="fa-solid fa-book mr-1.5"></i>Subject Teacher</span>
        </td>
        <td class="py-4 px-6" colspan="2">
          <span class="text-xs text-gray-500"><i class="fa-solid fa-calendar mr-1.5 text-gray-400"></i>${formatDate(teacher.assigned_at)}</span>
        </td>
      </tr>
    `;
  });
...
function displayAdvisoryList(advisories) {
  ...
  paginatedAdvisories.forEach((advisory) => {
    ...
    html += `
      <tr class="hover:bg-gray-50 transition">
        <td class="py-4 px-6" colspan="2">
          <div class="text-sm font-medium text-gray-900"><i class="fa-solid fa-people-group mr-2 text-[#043915]"></i>${escapeHtml(advisory.advisory_name)}</div>
          <div class="text-xs text-gray-500"><i class="fa-solid fa-chalkboard-user mr-1.5 text-gray-400"></i>Teacher: ${escapeHtml(advisory.teacher_name)}</div>
        </td>
        <td class="py-4 px-6">
          <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold ${gradeClass}"><i class="fa-solid fa-graduation-cap mr-1.5"></i>Grade ${advisory.grade_level}</span>
        </td>
        <td class="py-4 px-6" colspan="2">
          <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold ${capacityClass}"><i class="fa-solid fa-users mr-1.5"></i>${advisory.student_count}/40 Students</span>
        </td>
        <td class="py-4 px-6">
      ...
      html += `
        <div class="bg-gray-50 p-4 rounded-xl border border-gray-200 hover:shadow-md transition">
          <div class="flex items-start gap-3 mb-2">
            ...
            <div class="flex-1">
              <p class="font-bold text-gray-900"><i class="fa-solid fa-user mr-1.5 text-[#043915]"></i>${escapeHtml(student.student_name)}</p>
              <p class="text-xs text-gray-500"><i class="fa-solid fa-id-card mr-1 text-gray-400"></i>LRN: ${escapeHtml(student.lrn)}</p>
            </div>
            <span class="px-2 py-1 rounded-lg text-xs font-bold ${gradeClass}"><i class="fa-solid fa-layer-group mr-1"></i>Grade ${student.grade_level}</span>
          </div>
          <p class="text-xs text-gray-400"><i class="fa-solid fa-calendar mr-1 text-gray-400"></i>Assigned: ${formatDate(student.assigned_date)}</p>
        </div>
      `;
    });
    html += "</div>";
  ...
  modal.innerHTML = `
    <div class="bg-white w-full max-w-md rounded-2xl p-8 shadow-2xl relative animate-scale-in">
      ...
      <div class="bg-blue-50 rounded-xl p-4 mb-6 max-h-48 overflow-y-auto">
        <p class="text-xs font-bold text-blue-900 mb-2 uppercase tracking-wider"><i class="fa-solid fa-list-check mr-1.5"></i>Selected Students:</p>
...
function displayTableData(data) {
  ...
  paginatedData.forEach((row) => {
    ...
    html += `
      <tr class="hover:bg-gray-50 transition">
        <td class="py-4 px-6">
          <div class="text-sm font-medium text-gray-900"><i class="fa-solid fa-user mr-1.5 text-[#043915]"></i>${escapeHtml(row.student_name)}</div>
          <div class="text-xs text-gray-500"><i class="fa-solid fa-layer-group mr-1 text-gray-400"></i>Grade ${row.grade_level}</div>
        </td>
        <td class="py-4 px-6 text-sm text-gray-600"><i class="fa-solid fa-id-card mr-1.5 text-gray-400"></i>${escapeHtml(row.lrn || "N/A")}</td>
        <td class="py-4 px-6">
          <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold ${gradeClass}"><i class="fa-solid fa-graduation-cap mr-1.5"></i>Grade ${row.grade_level}</span>
        </td>
        <td class="py-4 px-6 text-sm text-gray-900"><i class="fa-solid fa-chalkboard-user mr-1.5 text-[#043915]"></i>${escapeHtml(row.teacher_name)}</td>
        <td class="py-4 px-6">
          <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-green-100 text-green-800"><i class="fa-solid fa-people-group mr-1.5"></i>${escapeHtml(row.advisory_name)}</span>
        </td>
        <td class="py-4 px-6 text-sm text-gray-600"><i class="fa-solid fa-calendar mr-1.5 text-gray-400"></i>${formatDate(row.assigned_date)}</td>
        <td class="py-4 px-6">
...
// ============================================================
// DOMContentLoaded — auto-load + live filters (no Apply button)
// ============================================================
document.addEventListener("DOMContentLoaded", function () {
  checkAdvisoryAvailability();

  // Load all records immediately on page open
  loadFilteredData();

  // Teacher type change triggers the correct view (advisory list / subject list / default)
  ...
  // Other filters reload the current view
